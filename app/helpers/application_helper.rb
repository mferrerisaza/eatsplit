module ApplicationHelper
  ...
  def card_photo_or_generic(user)
    if user == current_user && (user.profile.nil? || user.profile.photo.blank?)
      cl_image_tag "facebook-profile-picture-no-pic-avatar.jpg", class: "bill-card-user-avatar"
    elsif user == current_user
      cl_image_tag user.profile.photo, height: 300, width: 300, crop: :fill,gravity: :face, class: "bill-card-user-avatar"
  ...
  def table_card_photo_or_generic(user)
    if user == current_user && (user.profile.nil? || user.profile.photo.blank?)
